...
1. **GUIの練り直し** — 現状の配色・余白・アイコンは仮
2. **Service Workerの動作確認** — オフライン時に画面の骨組みが出るか。開発用の埋め込みブラウザでは登録が拒否され未検証
3. **初回で見送った機能の候補** — 期限のプッシュ通知(Web Push + Cron Trigger)、繰り返しタスク、タグ・分類。使ってみて必要になったら足す
