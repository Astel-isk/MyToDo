...
- タグの削除は絞り込みの「編集」から行う。押すとチップが削除の入口に変わり、
...
3. **既存タスクのタグを変えられない** — 画面では追加時にしかタグを選べない。付け替えは
   MCPの `update_task` か `PATCH /api/tasks/:id` からしかできない
4. **初回で見送った機能の候補** — 繰り返しタスク。使ってみて必要になったら足す
